...
public class TwelfthRoom implements RoomViewProvider {
    private static final int UNDERLAY_PIXEL_WIDTH = 1418;
    private static final int UNDERLAY_IN_CENTIMETERS_WIDTH = 350;
    private static final long UNDERLAY_ROOM_ID = 12L;
    ...
    public RoomView getRoom(String publicId, double paintingWidth, double paintingHeight) {
        ...
        long yValue = Math.round(-160 + ((paintingHeight - 59) / 71) * 80);
        long xValue = 0;
        if (paintingWidth <= 130 && paintingWidth > 59 && paintingHeight <= 130 && paintingHeight > 59) {
